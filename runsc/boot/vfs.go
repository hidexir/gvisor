...
func (c *containerMounter) mountSubmountsVFS2(ctx context.Context, conf *config.Config, mns *vfs.MountNamespace, creds *auth.Credentials) error {
	...
	for i := range mounts {
		...
		if mnt != nil && isReadOnly(conf, &submount.Mount) {
...
func (c *containerMounter) getMountNameAndOptionsVFS2(conf *config.Config, m *mountAndFD) (string, *vfs.MountOptions, error) {
	...
	opts.ReadOnly = isReadOnly(conf, &m.Mount)
	for _, o := range m.Options {
		...
		case "rw", "ro":
			// Already handled, don't send warning for them.
		default:
...
func isReadOnly(conf *config.Config, m *specs.Mount) bool {
	if conf.Overlay {
	...
	// Returns readwrite if not specified in options.
	return false
}
